# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 00:36:45 2024
...
class ExampleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # card_content = response.css('article.residential-card::text').get()
        # address = response.css('residential-card__address-heading::text').get()
        price = response.css('span.property-price ::text').get()
        yield {'titletext':price}
        # bed = response.css('aria-label.bedrooms::text').get()
        # bath = response.css('aria-label.bathrooms::text').get()
        # parking = response.css('aria-label.parking spaces::text').get()

        
        # yield {'full':card_content, 'address':address, 'price':price, 'bed':bed, 'bath':bath, 'parking':parking}
