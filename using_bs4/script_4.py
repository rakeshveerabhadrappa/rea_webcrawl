...
https://www.realestate.com.au/buy/in-manor+lakes,+vic+3024/list-1?activeSort=list-date&source=refinement
"""
...
def start_scrapping(_response, data):
    ...
    if _response.status_code == 200:
        ...
        for div in presentation_divs:
            ...
            data.append({'price':property_price, 
...
#%%
url = f'https://www.realestate.com.au/buy/in-{suburb},+{state}+pc/list-1?activeSort=list-date&source=refinement'
...
for n_page in tqdm(range(2,n_pages+1)):
    url = f'https://www.realestate.com.au/buy/in-{suburb},+{state}+pc/list-{n_page}?activeSort=list-date&source=refinement'
    response = requests.get(url, headers = headers)
    data = start_scrapping(response, data)
#%%
import pandas as pd
data_df= pd.DataFrame(data)
data_df.to_csv(f'../data/{suburb}_{state}_{pc}.csv', index = False)